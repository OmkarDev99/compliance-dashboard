...
print("Loading CA embedding model...")
embed_model = SentenceTransformer("BAAI/bge-base-en-v1.5")

chroma_client = chromadb.PersistentClient(path=CHROMA_DB_PATH)
collection = chroma_client.get_collection(name=COLLECTION_NAME)
print(f"CA collection loaded with {collection.count()} chunks.")
...
def build_prompt(question: str, chunks: list) -> str:
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        context_blocks.append(
            f"[{i}] (Source: {meta['source']}, Title: {meta['title']}, "
            f"Date: {meta['publication_date']})\n{chunk['text']}"
        )
    context_text = "\n\n".join(context_blocks)

    return f"""You are a Chartered Accountant compliance assistant. Answer ONLY using the
CONTEXT below, which consists of excerpts from NSE, RBI, SEBI, and Udyam compliance
notices, circulars, and directions. Do not use any outside knowledge. If the answer is
not present in the context, respond exactly:
...
def ask_gemini(prompt: str, max_retries: int = 2) -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_error = error
            print(f"Gemini call failed (attempt {attempt + 1}/{max_retries}): {error}")
...
def get_answer(question: str) -> dict:
    ...
    if not question:
        return {"answer": "Please enter a question.", "sources": [], "confidence": "invalid_input"}
    if len(question) > 500:
        return {
    ...
    try:
        ...
        print(f"CA retrieval failed: {error}")
        return {
    ...
    try:
        answer_text = ask_gemini(build_prompt(question, chunks))
    except Exception as error:
        print(f"CA generation failed: {error}")
        return {
    ...
    return {"answer": answer_text, "sources": sources, "confidence": "answered"}
