...
async def seed():
    ...
    async with async_session_maker() as db:
        result = await db.execute(select(User).filter(User.email == "admin@csdashboard.com"))
        ...
        db.add_all([admin, staff1, staff2, partner])
        await db.flush()  # Resolve user IDs
        ...
        rules = [
            ComplianceRule(
                ...
                frequency="annual",
                due_days_from_trigger=60,  # 60 days after FY end
            ...
            ComplianceRule(
                ...
                company_types=["private_limited", "public_limited", "opc"],
                frequency="annual",
                due_days_from_trigger=30,  # 30 days after AGM / FY end
        ...
        for comp in companies:
            count = await run_rule_engine_for_company(db, comp, user_id=admin.id)
